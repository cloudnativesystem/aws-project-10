...
locals {
  cluster_name = "datalake-${var.environment}-eks-${random_string.suffix.result}"
}
...
data "terraform_remote_state" "dataplatform-dev-vpc-us-east-2" {
  backend = "s3"
...
module "eks" {
  ...
  private_subnets         = data.terraform_remote_state.dataplatform-dev-vpc-us-east-2.outputs.private_subnets
  ...
  vpc_id = data.terraform_remote_state.dataplatform-dev-vpc-us-east-2.outputs.vpc_id
